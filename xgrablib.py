...
class xgrabFromdb:
    ...
    def _end_metaTag(self, beginTime=None,  organisatie=1 , bewerking=3 ):
        EINDMETADATA = etree.Element('EINDMETADATA')
        if beginTime == None:
            etree.SubElement(EINDMETADATA , "TIJD").text = datetime.datetime.now().isoformat()
        elif beginTime.__class__ == datetime.datetime:
            etree.SubElement(EINDMETADATA , "TIJD").text = beginTime.isoformat()
        else :
            etree.SubElement(EINDMETADATA , "TIJD").text = str( beginTime )
